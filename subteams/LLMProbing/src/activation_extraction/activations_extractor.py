...
class ActivationsExtractor():
    """
    A utility class for extracting and storing activations from a transformer model during forward passes.

    This class registers forward hooks on specified layers of an encoder-decoder transformer model to capture
    activations and save them to disk for later analysis.

    Methods:
    - hook_fn(module, input, output, layer_name, layer_outputs):
        Stores the activation output of a layer to a dictionary.
    - register_hooks(model_part, part_name, layer_outputs, layers_to_extract):
        Registers hooks on the specified layers of a given model component (encoder/decoder).
    - extract_activations(dstr, samples_path, activations_path, layers_to_extract=['ffn']):
        Runs activation extraction for all samples in the dataset and saves the results.

    """

    def __init__(self):
        """
        Initialises the ActivationsExtractor instance.
        """
        pass

    def hook_fn(self, module, input, output, layer_name, layer_outputs):
        """
        Hook function to store the output of a specified layer.

        Args:
        - module (torch.nn.Module): The module being hooked.
        - input (tuple): The input tensor(s) to the module.
        - output (torch.Tensor): The output tensor from the module.
        - layer_name (str): The name of the layer being hooked.
        - layer_outputs (dict): A dictionary to store the extracted activations.

        Returns:
        - None: The function updates `layer_outputs` in place.
        """
    ...
    def extract_activations(self, dstr, samples_path, activations_path, layers_to_extract=['ffn']):
        """
        Extracts activations from specified layers while running inference on a dataset.

        Args:
        - dstr (object): The transformer (odeformer) model.
        - samples_path (str): Path to the directory containing sample input data.
        - activations_path (str): Directory where extracted activations will be saved.
        - layers_to_extract (list of str, optional): List of layer types to extract activations from.
          Default is ['ffn'] (FeedForward layers).

        Returns:
        - None: Extracted activations are saved to disk.

        Notes:
        - Hooks are registered in both the encoder and decoder before inference.
        - Activations are stored in `.pt` files, using sample IDs in the filename.
        - If an activation file for a sample already exists, it is skipped.
        - Computes and stores the R^2 score for each sample's predicted trajectory.
        - Also stores the predicted symbolic expression generated by the model.
